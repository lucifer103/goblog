...
// baseURL -- 用以分页链接
// PrePage -- 每页条数，传参为小于或者等于 0 时为默认值 10，可通过 config/pagination.go 修改
func New(r *http.Request, db *gorm.DB, baseURL string, PrePage int) *Pagination {

	// 默认每页数量
	if PrePage <= 0 {
		PrePage = config.GetInt("pagination.prepage")
	}
